...
def shooting_dirichlet1(d2theta, d2phi, interval, bcs, N, fg, fx):
    ...
    def rhs_shooting(s, z):
        ...
        return np.array([dtheta, dphi, d2theta(s, theta, dtheta, phi, dphi, fg, fx), d2phi(s, theta, dtheta, phi, dphi, fg, fx)])
    def residuals_shooting(guesstheta, guessphi):
        """
        Solve IVP given y'(a): compute r = y(b) - bcs[1]
    ...
    guesstheta = scipy.optimize.root(residuals_shooting, [0,1], 0)
    slists = np.linspace(interval[0], interval[1], N+2)
    sol_shoot = solve_ivp(rhs_shooting, interval, [bcs[0], bcs[2], guesstheta], t_eval=slists)
    return slists, sol_shoot.y    
    ...
    for j in theta0:
        for k in phi0:
            ...
            def right(s, z):
                ...
                #return np.array([dx1(theta, phi), dy(theta, phi), dz1(theta, phi)...])
            hair_xandyandz = solve_ivp(right, [0, L], [x0,y0,z0,hair[0][0], hair[1][0], hair[2][0]])
...
def task4(L,R):
    hairs = drawing_hair1(L, R, 0.05, 0.1, np.linspace(0, 0.49*np.pi, 100), np.linspace(0, np.pi, 100))
    ...
    for j in hairs:
        plt.plot(j[0], j[1], j[2], 'k')
    plt.xlim()
    plt.ylim()
    plt.title("3d plot of hairs on the x,y and z planes")
