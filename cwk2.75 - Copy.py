...
def shooting_dirichlet1(d2theta, d2phi, interval, N, theta0, phi0, fg, fx):
    """
    Solve a BVP using shooting
    ...
    def d2theta1(q, s, fg, fx, phi):
        theta, dtheta = q
        return np.array([dtheta, d2theta(s, theta, phi, fg, fx)])
    
    def d2phi1(q, s, fg, fx, theta):
        phi, dphi = q
        return np.array([dphi, d2phi(s, theta, phi, fg, fx)])
    
    def solve(guess, f, g):
        init = [f, guess]
        sol1 = odeint(g, init, slists, args = (fg,fx,theta0))
        return(sol1[-1,1]-0)
    
    guess1 = scipy.optimize.brentq(solve, -10, 10, args = (theta0, d2theta1))
    guess2 = scipy.optimize.brentq(solve, -10, 10, args = (phi0, d2phi1))
    
    initial = [theta0, guess1, phi0, guess2]
    ...
    soln = odeint(rhs_shooting, initial, slists, args=(fg,fx))
    return slists, soln
...
def drawing_hair1(L, R, fx, fg, theta0, phi0):
    ...
    for j in theta0:
        for k in phi0:
            hairs, hair = shooting_dirichlet1(d2theta, d2phi, [0,L], 100, theta0, phi0, fg, fx)
            x0 = R*np.cos(j)*np.cos(k)
            y0 = -R*np.cos(j)*np.sin(k)
            z0 = R*np.sin(j)
            def right(s, z):
                x,y,z,theta,dtheta,phi,dphi = z
                return np.array([dx1(theta, phi), dy(theta, phi), dz1(theta), dtheta, dphi, d2phi(s, theta, phi, fg, fx), d2theta(s, theta, phi, fg, fx)])
            #hair_xandyandz = solve_ivp(right, [0, L], [x0,y0,z0,hair[0][0], hair[1][0], hair[2][0], hair[3][0]])
            initial = [x0, y0, z0, theta0, hair[0, 1], phi0, hair[0, 3]]
            interval = np.linspace(0, L, 100)
            hair_xandyandz = odeint(right, initial, interval, args=(fg, fx))
